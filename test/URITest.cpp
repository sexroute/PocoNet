...
TEST(URITest, testOther)
{
	...
	EXPECT_TRUE(uri.getQuery() == "q=foo&bar");
	EXPECT_TRUE(uri.getRawQuery() == "q=foo&bar");
	EXPECT_TRUE(uri.toString() == "http://google.com/search?q=foo&bar#frag%20ment");
	EXPECT_TRUE(uri.getPathEtc() == "/search?q=foo&bar#frag%20ment");

	uri.setQuery("q=foo/bar");
	EXPECT_TRUE(uri.getQuery() == "q=foo/bar");
	EXPECT_TRUE(uri.getRawQuery() == "q=foo%2Fbar");
	EXPECT_TRUE(uri.toString() == "http://google.com/search?q=foo%2Fbar#frag%20ment");
	EXPECT_TRUE(uri.getPathEtc() == "/search?q=foo%2Fbar#frag%20ment");
	...
	EXPECT_TRUE(uri.getRawQuery() == "q=pony%7eride&pa%3Dra%26m1=");
	EXPECT_TRUE(uri.getQuery() == "q=pony~ride&pa=ra&m1=");
	uri.addQueryParameter("pa=ra&m2", "val&ue");
	EXPECT_TRUE(uri.getRawQuery() == "q=pony%7eride&pa%3Dra%26m1=&pa%3Dra%26m2=val%26ue");
	EXPECT_TRUE(uri.getQuery() == "q=pony~ride&pa=ra&m1=&pa=ra&m2=val&ue");

	uri = "http://google.com/search?q=hello+world#frag%20ment";
	EXPECT_TRUE(uri.getHost() == "google.com");
	EXPECT_TRUE(uri.getPath() == "/search");
	EXPECT_TRUE(uri.getQuery() == "q=hello+world");
	EXPECT_TRUE(uri.getRawQuery() == "q=hello+world");
	EXPECT_TRUE(uri.getFragment() == "frag ment");
	EXPECT_TRUE(uri.toString() == "http://google.com/search?q=hello+world#frag%20ment");
	EXPECT_TRUE(uri.getPathEtc() == "/search?q=hello+world#frag%20ment");
}

TEST(URITest, testEncodeDecode)
{
	std::string str;
	URI::encode("http://google.com/search?q=hello+world#frag ment", "+#?", str);
	EXPECT_TRUE (str == "http://google.com/search%3Fq=hello%2Bworld%23frag%20ment");

	str = "";
	URI::encode("http://google.com/search?q=hello+world#frag ment", "", str);
	EXPECT_TRUE (str == "http://google.com/search?q=hello+world#frag%20ment");
	
	str = "";
	URI::decode("http://google.com/search?q=hello+world#frag%20ment", str, true);
	EXPECT_TRUE (str == "http://google.com/search?q=hello world#frag ment");
...
TEST(URITest, testQueryParameters)
{
	...
	EXPECT_TRUE (params[0].first == "q");
	EXPECT_TRUE (params[0].second == "hello world");
	EXPECT_TRUE (params[1].first == "client");
	EXPECT_TRUE (params[1].second == "safari");

	uri.setQueryParameters(params);
	EXPECT_TRUE (uri.toString() == "http://google.com/search?q=hello%20world&client=safari");

	uri = "http://google.com/search?q=&client&";
	params = uri.getQueryParameters();
	EXPECT_TRUE (params.size() == 2);
	EXPECT_TRUE (params[0].first == "q");
	EXPECT_TRUE (params[0].second == "");
	EXPECT_TRUE (params[1].first == "client");
	EXPECT_TRUE (params[1].second == "");

	uri.setQueryParameters(params);
	EXPECT_TRUE (uri.toString() == "http://google.com/search?q=&client=");

	params[0].second = "foo/bar?";
	uri.setQueryParameters(params);
	EXPECT_TRUE (uri.toString() == "http://google.com/search?q=foo%2Fbar%3F&client=");

	params[0].second = "foo&bar";
	uri.setQueryParameters(params);
	EXPECT_TRUE (uri.toString() == "http://google.com/search?q=foo%26bar&client=");
}
