...
void testLimitsSigned()
{
    ...
    TS i;
    EXPECT_THROW(i = da.convert<TS>(), RangeException);
...
void testContainerIterator()
{
    ...
    for (; it != end; ++it)
    {
        ...
        case 3:
            ...
            break;
        default:
            EXPECT_FALSE("must fail - wrong size");
        }
    }
}

TEST(VarTest, testInt8)
{
	Poco::Int8 src = 32;
	Var a1 = src;
    ...
    Int16 value2; 
    EXPECT_THROW(value2 = a1.extract<Int16>(), Poco::BadCastException);
...
TEST(VarTest, testInt16)
{
    ...
    Int32 value2; 
    EXPECT_THROW(value2 = a1.extract<Int32>(), BadCastException);
...
TEST(VarTest, testInt32)
{
    ...
    Int16 value2;
    EXPECT_THROW(value2 = a1.extract<Int16>(), BadCastException);
...
TEST(VarTest, testUDT)
{
    ...
    //float f;
    //EXPECT_THROW( f = da1£¬ Poco::BadCastException);
}

TEST(VarTest, testConversionOperator)
{
	Var any("42");
	int i = any;
	assert (i == 42);
...
TEST(VarTest, testComparisonOperators)
{
	Var any1 = 1;
	Var any2 = "1";
	assert (any1 == any2);
...
TEST(VarTest, testArithmeticOperators)
{
	Var any1 = 1;
	Var any2 = 2;
	Var any3 = any1 + any2;
    ...
    EXPECT_THROW(any3 = any1 - any2, InvalidArgumentException);
    EXPECT_THROW(any3 -= any2, InvalidArgumentException);
    EXPECT_THROW(any3 = any1 * any2, InvalidArgumentException);
    EXPECT_THROW(any3 *= any2, InvalidArgumentException);
    EXPECT_THROW(any3 = any1 / any2, InvalidArgumentException);
    EXPECT_THROW(any3 /= any2, InvalidArgumentException);
    ...
    EXPECT_THROW(++any1, InvalidArgumentException);
    EXPECT_THROW(any1++, InvalidArgumentException);
    EXPECT_THROW(--any1, InvalidArgumentException);
    EXPECT_THROW(any1--, InvalidArgumentException);
}

TEST(VarTest, testLimitsInt)
{
...
TEST(VarTest, testLimitsFloat)
{
	if (std::numeric_limits<double>::max() != std::numeric_limits<float>::max())
	{
		...
		Var da = iMin * 10;
        float f;
        EXPECT_THROW(f = da, RangeException);
...
TEST(VarTest, testArrayIdxOperator)
{
	std::string s1("string");
	Poco::Int8 s2(-23);
	Poco::Int16 s3(-33);
...
TEST(VarTest, testDynamicStructBasics)
{
...
TEST(VarTest, testDynamicStructString)
{
	DynamicStruct aStruct;
...
TEST(VarTest, testDynamicStructInt)
{
...
TEST(VarTest, testDynamicPair)
{
	Pair<int> aPair;
	assert (0 == aPair.first());
    EXPECT_THROW(aPair.second().convert<std::string>(), InvalidAccessException);
...
TEST(VarTest, testArrayToString)
{
	std::string s1("string");
	Poco::Int8 s2(23);
	std::vector<Var> s16;
...
TEST(VarTest, testArrayToStringEscape)
{
...
TEST(VarTest, testStructToString)
{
	DynamicStruct aStruct;
	aStruct["First Name"] = "Junior";
	aStruct["Last Name"] = "POCO";
...
TEST(VarTest, testStructToStringEscape)
{
	DynamicStruct aStruct;
...
TEST(VarTest, testArrayOfStructsToString)
{
	std::vector<Var> s16;
	DynamicStruct aStruct;
	aStruct["First Name"] = "Junior";
...
TEST(VarTest, testStructWithArraysToString)
{
	std::string s1("string");
	Poco::Int8 s2(23);
	std::vector<Var> s16;
...
TEST(VarTest, testJSONDeserializeString)
{
	Var a("test");
...
TEST(VarTest, testJSONDeserializePrimitives)
{
	Poco::Int8 i8(-12);
...
TEST(VarTest, testJSONDeserializeArray)
{
	Poco::Int8 i8(-12);
	Poco::UInt16 u16(2345);
	Poco::Int32 i32(-24343);
...
TEST(VarTest, testJSONDeserializeComplex)
{
	Poco::Int8 i8(-12);
	Poco::UInt16 u16(2345);
	Poco::Int32 i32(-24343);
...
TEST(VarTest, testJSONDeserializeStruct)
{
	Poco::Int8 i8(-12);
	Poco::UInt16 u16(2345);
	Poco::Int32 i32(-24343);
...
void testGetIdxNoThrow(Var& a1, std::vector<Var>::size_type n)
{
	Var val1 = a1[n];
}


void testGetIdxMustThrow(Var& a1, std::vector<Var>::size_type n)
{
	try
	{
		Var val1 = a1[n]; 
		EXPECT_FALSE("bad cast - must throw");
	...
	try
	{
        ...
        EXPECT_FALSE("bad const cast - must throw");
...
TEST(VarTest, testEmpty)
{
    ...
    EXPECT_THROW(i = da, InvalidAccessException);
    EXPECT_THROW(i = da.extract<int>(), InvalidAccessException);

}

TEST(VarTest, testIterator)
{
	Var da;
	assert (da.isEmpty());
	...
	it++;
	assert (it == end);
    EXPECT_THROW(++it, RangeException);
